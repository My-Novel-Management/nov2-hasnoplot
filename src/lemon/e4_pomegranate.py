...
def sc_meeting_after(w: World):
    ...
    return w.scene("会議（決定編）",
            ...
            sana.explain("タイトルは『$master_shortnovel1』とある"),
            sana.explain("会議室内の空気はぴんと張り詰め、誰もが初めて目にするその物語に入り込んでいるのが分かった"),
            sana.explain("ただ$Sの内心は穏やかじゃない",
                "まるで梶井基次郎の短編『檸檬』のように、先生の手によってここに置いていかれた想像の爆弾を抱えているみたいで、",
                "コピーを捲りながらもその手が燃えてしまうのではないかと思うような熱を感じている"),
            sana.think("何を書くか、プロットすら見せてもらえず、結局今こうして原稿を手に取るまで$noto先生がどんなジャンルで勝負しようとしているのか、",
                "それすら分からなかった",
                "$Sはてっきり今までずっと先生が書いてきた現代や昭和期の日本を舞台とした作品で、従来よりもやや文体を柔らかくした日常ものか恋愛ものだろうと踏んでいたのだけれど、",
                "まさか最近流行の兆しのある和風ファンタジィを書いてくるとは想像を大きく超えている"),
            sana.think("――けど、これって、ひょっとして"),
            king.talk("いやあ、びっくりしたね。しかし、何だ", "今までうちで扱ったことがあるタイプの作品ではないものの、流石の筆力に思わず唸ったよ"),
            sana.do("編集長が珍しく作品を褒めた",
            ...
            yone.do("$CSの言葉に、$yonezawaは眼鏡を少し持ち上げて口元に笑みを作る"),
...
def sc_formallyeditor(w: World):
    ...
    return w.scene("正式に担当に",
            ...
            sana.talk("そういえば先生、あの『$master_shortnovel1』の主人公のモデルって"),
            sana.think("不死身の少女がバケモノたちと戦う、というその大枠は、実は$Sが書いてきた短編小説の大枠と同じだった"),
            noto.do("だがその問いかけに先生は首を傾げただけだ"),
            sana.talk("あら"),
            sana.do("いつの間にかやってきた$mascatが$Sの脹脛に頬を擦り付け、猫缶の催促を始める"),
            sana.talk("先生、まだ今日は上げてないんですか？"),
            noto.talk("君の為に出かけていたんだから仕方ないだろう",
                "それよりも、そうだ。君の小説だがね、",
                "あれ、赤入れておいたから",
                "あとで読み返して修正しておくように"),
            sana.talk("え？"),
            sana.do("先生はテーブルの上の茶封筒を指差し、$Sに持って帰るように言った",
                "手に取って中を見てみると、確かに大量に赤が入れられた自分の短編小説のゲラで、書き上げたらそれで終わりじゃない、という作家側の気持ちが少しだけ分かった気がした"),
