...
def sc_badwriter(w: World):
    ...
    return w.scene("悪い作家",
            ...
            sana.think("両拳をぎゅっと握りしめ、今日こそは、という気合をこの小春日和の太陽から分けてもらうんだ、という眼差しである",
                "しかしインタフォンを何度押しても返答がないことが、$Sの苛立ちゲージを容易に頂点まで押し上げていた"),
            ...
            sana.talk("先生、原稿まだですか？",
                "返事がないなら勝手に開けますよ？"),
            sana.do("ドアに耳を済まし、中から反応がないことを確かめると、$Sはバッグから合鍵を取り出して容赦なく鍵穴に差し込む"),
            sana.think("あれ？　開いてる……"),
...
def sc_aboutplot(w: World):
    ...
    return w.scene("プロットとは",
            ...
            sana.talk("原稿がまだなことは理解しました",
                ...
                "今日はそれで引き下がりますから"),
            noto.talk("プロット？"),
            noto.do("何が引っかかったのか、先生は大きな声でそう繰り返し、$CSを見て目を細くした"),
            ...
            noto.do("先生はテーブルの端に追いやっていた原稿用紙を一枚千切ると、それを裏返しにして$CSに渡す"),
            sana.talk("シンデレラですか……"),
            sana.explain("それは誰もがよく知るおとぎ話だ",
                "父親が再婚してやってきた継母とその連れ子である義姉により、シンデレラはいじめられる",
                "けれどある時城で舞踏会が開かれ、どうしてもそれに行きたかった彼女は魔女の力を借りて綺麗なドレスやガラスの靴、カボチャの馬車などを手に入れ、",
                "舞踏会で憧れの王子様と踊ることになる",
                "しかし魔女の力は十二時で消えてしまう",
            ...
            sana.do("溜息をつきたいのは自分の方だ、という思いを呑み込んで$Sは言ったが、"),
            noto.talk("ない"),
            noto.do("先生は即答だ"),
