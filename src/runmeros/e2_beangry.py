...
def sc_myteam(w: World):
    ...
    return w.scene("同僚たち",
            ...
            king.be("恰幅の良いチェック柄のジャケットを着た男性が、鼻の下の髭を歪ませて$CSを見ていた"),
            sana.think("ああ、やはり機嫌が悪いのか","&"),
            sana.do("そんな諦めを心の中にそっと仕舞い、改めて彼女は「おはようございます」と挨拶をする",
                "その脇で副編集長の$full_yonezawaがスマートフォンを肩で挟みながら、何やら取次と数の相談をしていた","&"),
            yone.be("#仕事をしている"),
            yone.wear("シャツにベスト、ストライプのズボン、ネクタイ"),
            yone.look("こちらもアロハ柄のシャツに青と白のストライプパンツと、何とも目がちかちかしそうな服装なのだが、",
                "口調の丁寧さと判断の冷静さからそこにはアンドロイドぽさを感じる", "実際他の人間からは密かに「ヨネロイド」と呼ばれていた"),
            sana.talk("ども"),
            sana.do("一瞬目が合って軽く頭を下げられ反射的に会釈する",
            ...
            sana.think("今の発言を録音して労基署にでも持ち込もうかと思うことは、日に一度では利かない",
                ...
                "その王が何故か今、朝休むと言ってきた$nirasakiへの憤りを$Sにぶつけているのだ"),
            sana.talk("今日も一日がんばります！",
                "それでは仕事がありますので！"),
            sana.do("空元気で右腕を上げて編集長を呆れさせると、自分のデスクにそそくさと向かう",),
            sana.explain("$Sの机は編集長から左手側の島にある。ちょうどカタカナのコの字を窓に投げつけたような形の配置で下側の線の端がそうだった", "&"),
            fukaya.be("その反対側で慌ただしくコピー機からプリントアウトを机に運び込んでいるのは、尊敬する先輩の$full_fukayaだ"),
            fukaya.wear("ゆったりしたディープグリーンのロングワンピにストレッチ素材のパンツ"),
            sana.talk("先輩、おはようございます", "何の資料ですか？"),
            sana.do("積み上げられた辞書二冊分くらいの資料の中には、よく見ると手書きのものも混ざっている"),
            ...
            sana.do("$Sの隣の席にいつもいるはずのボサボサ頭の眼鏡が急遽出勤することになった元凶だったが、",
                "今日は不在だ","最近体調悪いと高級マスクにやたらと厚着をしていたけれど、まさかインフルエンザだったとは",
            ...
            sana.think("$Sは小さい頃から体の丈夫さだけは自慢できた",
                ...
                "実際$fukayaが誘ってくれたのもバタバタと辞めて人手不足になったからだと聞いている"),
...
def sc_request(w: World):
    ...
    return w.scene("先輩の頼み",
            ...
            sana.talk("やっぱり休んだ方が良いですよ",
                "先輩いない間はみんなで何とかします"),
            fukaya.talk("もうちょっと無理利く体だと思ってたんだけど、妊婦って意外と大変なのね",
                "担当してる先生方には途中離脱みたいな形になって申し訳ないなと思うけど、",
                "こればっかりは仕方ないか"),
            sana.think("いつも自分よりも作家や本、作品を大事に、というその姿勢は素晴らしいと思いつつも、",
                "ふと我に返った時に自分はそこまで出来るだろうか、という疑問が湧いてくる",
                "果たして今の$Sに自分の人生や命を懸けてまで本作りに取り組むことが出来るのだろうか"),
...
def sc_emergency(w: World):
    ...
    return w.scene("緊急事態",
            ...
            fukaya.talk("うん、そうなの",
                "ちょっとね、ワケアリで"),
            fukaya.do("先輩は悪戯っぽく笑みを浮かべると、$Sの手からプロフィールを受け取って用意してあった封筒に入れる"),
            ...
            sana.talk("先輩、大丈夫ですか？"),
            fukaya.talk("ごめん。ちょっと、無理だ"),
            sana.talk("え？　せ、先輩！？"),
            fukaya.do("$fukayaはその場に体を丸めて蹲る"),
            sana.talk("あ、あの！　先輩が！"),
            yone.come("#やってきて"),
            yone.talk("$fukayaさん？　どうしたんです？"),
            yone.do("$CSの声にすぐ反応した$yonezawaがやって来て、$fukayaの背中を擦る"),
            ...
            yone.do("そう言って彼は電話を取ると外線に切り替えて手早く番号を押した"),
