...
def sc_running(w: World):
    ...
    return w.scene("走れ",
            ...
            _.explain("$Sはただの平の編集者である。それもまだ転籍してきて二ヶ月と新人に等しい平の編集者である", "&"),
            ...
            sana.do("荒い息をつきながら必死に駅へと向かう",
                "ピンクの蛍光ラインがキュートだと思ってこの夏購入したスニーカーは既によれよれになり、特に右足のソールが削れて時々滑るようになっていた",
                "既に日は高くスマートフォンを見れば時刻は十時に届こうかという頃合いだ",
                "通りに出るとこんな時間帯でも人通りが多いのはやはり地元と違う都内という場所の特性だろう",
                "$Sのようなスーツ姿の人もいるが、秋物コートやパーカー、カジュアルな重ね着で悠然と歩く女性に、スポーツウェアを着込んでいる都民ランナーについ目が向かう"),
            ...
            road.look("#駅前の混雑した歩道"),
            sana.look("すれ違った自転車のおばさんが$Sを見て一瞬目を丸くしていたが、",
            ...
            sana.talk("あ……"),
            sana.do("ついさっきまで青空が覗いていたと思ったら今の$Sの心境のような雲行きへの急変から、激しく雨が落ち始めた"),
            sana.talk("もうなんて日！"),
            sky.look("#晴れていたと思ったら雨が降り始める"),
            sana.go("$Sは慌てて傘を買いにコンビニに駆け込んだ"),
            ## NOTE:
            ##  走れメロスを意識した冒頭
...
def sc_inconveni(w: World):
    ...
    return w.scene("コンビニにて",
            ...
            sana.hear("鼻息荒い$Sを、いつもの抑揚のないコンビニ店員の声が出迎える"),
            inside.look("意外と店内は混み合っていて、$Sの後から入ってきた人はビニル傘だけ購入してさっさと出ていく", "&"),
            sana.do("とりあえず雑誌コーナーに立って市場調査をしつつ、ガラス越しにスコールが止まないかと見ていたが、雷まで鳴り始めてそれが望み薄だ諦めろと怒鳴られているように感じられた"),
            sana.do("スマートフォンで電車の時刻を調べ、会社までの時間を計算する",
                "できれば五分後の一本は逃したくない"),
            sana.explain("意外と知られていないが、コンビニには売れ筋の文庫本も置かれていて、それが結構売上に繋がっている",
                "ただ大手出版社のものばかりで$Sのところのような小さな会社が棚を確保するなんて、夢のまた夢だった",
            ...
            shelf.look("$Sはそこに並ぶ$weekmagazine1を手に取る",
                "芸能人や有名人のスキャンダル、不倫問題をよく取り上げて話題にすることが多い雑誌で、普段あまり読むことはないのだけれど、",
                "そこに不倫や汚職という言葉に続いて並んでいた小さな『盗作騒動！？』と赤字に目を留めてしまう"),
            w.eventStart("大手の盗作騒動"),
            sana.do("ページを捲り該当の部分を探そうとしたが、後ろからジャージ姿の男性が同じ雑誌を籠に入れていったのを見て、$Sはそれを棚に戻した",
                "ドリンクコーナーへ向かう",
                "時間がある時は新作チェックを欠かさないのだけれど、スマートフォンが鳴り、$lineで$fukaya先輩から『急がなくても大丈夫だから』とわざわざ入ったので$Sは踵を返し、",
                "急いでレジに回って傘とホットレモネードを買って店の外に出た"),
            sana.talk("先輩、待ってて下さいよ！", "$full_sana、今参ります！"),
            sana.do("勢い良く傘を開くと、パラパラとした音になって、突如雨が止んでしまった"),
...
def sc_mycompany(w: World):
    ...
    return w.scene("私の会社です",
            ...
            sana.do("そんな$Sを後ろから追い越してビルへと入っていく人が何人かいる",
                "出版社というのはどうも一般の会社よりもフレキシブルな出勤体系になっているのか、信じられないくらい遅く出社してくる人もまま見かける"),
...
def sc_myoffice(w: World):
    ...
    return w.scene("編集部です",
            ...
            wall.look("蛍光灯が半分になってやや暗い廊下が真っ直ぐに伸びていて、オフィスを出入りする人が行き来している",
            ...
            wall.look("それはライトノベルレベールの大手$w_dengekiで当代一の人気を誇る『$isesui』のポスターだった", w.poster_isesui,
                ...
                "編集長がいつも$w_isesuiくらいの大ヒット作を発掘してこいと、企画会議がある度に唾を飛ばして言う"),
            w.comment("イセスイの簡単な雰囲気描写。ポスターの絵柄でそれっぽく。できればこのすば的なの"),
            sana.think("確かにキャラクター同士のやり取りは面白いが、それとは別の方向を考えたいと、個人的には思っている"),
            ...
            sana.hear("編集長だ", "その特有の濁声はパーティションを倒しそうな勢いでオフィスに響いていた"),
