...
def sc_beout(w: World):
    ...
    return w.scene("先生は不在",
            ...
            sana.do("この日は気分転換も兼ねて、久しぶりに社用車を使わせてもらって、都内を走る",
                "免許は大学時代に取っておいて良かったけれど、今にして思えばよくマニュアル免許なんて選んだなと$nirasakiにも指摘された"),
            ...
            sana.talk("え？"),
            sana.do("一枚だけ千切られた原稿用紙が消しゴムで留め置かれていて、そこには先生の手書きの地図付きである店の名が書かれていた"),
            camera=w.sana,
            stage=w.on_hisapart,
...
def sc_nearrestaurant(w: World):
    ...
    return w.scene("待ち合わせの洋食屋に向かう路地",
            ...
            sana.do("六階に満たないマンションが立ち並び、コンビニやドラッグストアはあるがあまり他の店は見当たらない",
                "十字路にあったコンビニで店の場所を聞いてみると、坂を降りたところだと教わり、お礼を言ってレジのところに置かれていたホットのレモンティーを購入した"),
